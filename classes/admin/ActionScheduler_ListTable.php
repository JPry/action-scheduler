...
class AS_List_Table extends WP_List_Table {
	...
	function __construct() {
		...
		parent::__construct( array(
			'singular' => 'scheduled-action',
			'plural'   => 'scheduled-actions',
			'ajax'     => false
		) );
	...
	/**
	 * Output the content for each column.
	 *
	...
	function column_default( $item, $column_name ) {
		...
		switch ( $column_name ) {
			...
			case 'hook':
				$column_content = sprintf( '<code>%s</code> %s', sanitize_title( $item->get_hook() ), $this->row_actions( $this->get_actions( $item ) ) );
				break;

			case 'status':
				$column_content = sprintf( '<mark class="%s">%s</mark>', sanitize_title( 'status' ), 'status' );
				break;

			case 'args' :
				foreach( $action->get_args() as $key => $value ) {
					$column_content = sprintf( "<code>%s => %s</code><br/>", $key, $value );
				}
				break;
			...
			case 'recurrence':
				...
				if ( is_callable( array( $schedule, 'interval_in_seconds' ) ) ) {
					$column_content = as_human_interval( $recurrence->interval_in_seconds() );
				} else {
					$column_content = __( 'Non-repeating', 'action-scheduler' );
				}
				break;

			case 'scheduled':

				$column_content = get_date_from_gmt( gmdate( 'Y-m-d H:i:s', $next_timestamp ), 'r' );

				if ( gmdate( 'U' ) > $next_timestamp ) {
					$column_content .= sprintf( __( ' (%s ago)', 'action-scheduler' ), as_human_interval( gmdate( 'U' ), $next_timestamp ) );
				} else {
					$column_content .= sprintf( __( ' (%s)', as_human_interval( gmdate( 'U' ), $next_timestamp ) );
				}
				break;
	...
	/**
	 * Get the actions displayed under the hook name for a specific action
	 * 
	 * @param array $item A singular item (one full row's worth of data)
	 * @return array
	 * @since 1.6.0
	...
	private function get_actions( $item ) {
		...
		$action_url = add_query_arg(
			array(
				...
				'_wpnonce'     => wp_create_nonce( $item['action_id'] )
			)
		);

		// Preserve current status filter after processing the action
		if ( isset( $_REQUEST['status'] ) ) {
			$action_url = add_query_arg( array( 'status' => $_REQUEST['status'] ), $action_url );
		}

		$all_statuses = array(
			'process' => __( 'Run', 'action-scheduler' ),
			'trash'   => __( 'Trash', 'action-scheduler' ),
		);

		foreach ( $all_statuses as $status => $label ) {
			$actions[ $action ] = sprintf( '<a href="%s">%s</a>', esc_url( add_query_arg( 'action', $status, $action_url ) ), $label );
		}

		if ( $item['status'] == ActionScheduler_Store::STATUS_COMPLETE ) {
			unset( $actions['trash'] );
		} elseif( ! in_array( $item['status'], array( 'cancelled', 'expired', 'switched', 'suspended' ) ) ) {
			unset( $actions['trash'] );
		}

		return apply_filters( 'action_scheduler_list_table_actions', $actions, $item );
	}
